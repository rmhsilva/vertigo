//Ported to mbed by Phil Crump phildcrump@gmail.com 2013
//from RFM22 Lib - edited by James Coxon jacoxon@googlemail.com 2011
#include "RFM22.h"
...
void rfm22::read(uint8_t start_addr, uint8_t buf[], uint8_t len) {
	...
	// make sure the msb is 0 so we do a read, not a write
	start_addr &= 0x7F;
	rfm.write(start_addr);
	for (int i = 0; i < len; i++) {
		buf[i] = rfm.write(0x00);
	}
...
void rfm22::initSPI() {
	rfm.format(8,0); // 8 bits per frame, mode 0
	rfm.frequency(8000000); // 8MHz
