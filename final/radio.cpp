...
// RFM22 868
DigitalOut ss_port_868(p8);
SPI rfm868(p5, p6, p7);

// RFM22 434
DigitalOut ss_port_434(p14);
SPI rfm434(p11, p12, p13);
...
static void resetFIFO_rfm434() {
    write_rfm434(0x08, 0x03);
    write_rfm434(0x08, 0x00);
}

static void setInterrupt_rfm434(int interrupt, int is_on) {
...
static int readAndClearInterrupts_rfm434() {
...
static void resetFIFO_rfm868() {
    write_rfm868(0x08, 0x03);
    write_rfm868(0x08, 0x00);
}

static void setInterrupt_rfm868(int interrupt, int is_on) {
    //high bits of interrupt are 0x03/0x05, low are 0x04/0x06
    char high = (interrupt >> 8) & 0xff;//    0x03/0x05
    char high_is_on = (is_on >> 8) & 0xff;
    char low = interrupt & 0xff;//        0x04/0x06
...
static int readAndClearInterrupts_rfm868() {
    //high bits are 0x03, low are 0x04
...
static void rtty_434_txbit (int bit)
{
        if (bit)
        {
          // high
...
static void rtty_868_txbit (int bit)
{
        if (bit)
        {
          // high
...
static int crc_xmodem_update (int crc, char data)
    {
        int i;
